...
%
% mYode: a matrix, one row one time point
% mYobs: a matrix, one row one time point
% t_frame: a row vector indicating the time points of the data
%
%
%
%
...
function [ ps, ps_trial, chi2s, chi2s_trial, acceptance ] = gen_GibbsWithMH(t_frame, mYobs, mYode, startp, muprior,sigmaprior, indexFit, nruns, nburnin, nthinning,Xd,observed,MoleculeNumberInOneNanoMole,ode_fun)
...
n_T = length(t_frame); % number of time points in data;
N_tmp = length(startp);
n_Y = size(mYode,2);                  %n_Y = length(Yode); 
...
mYode_tmp = Ydd_tmp(t_frame,observed)./ MoleculeNumberInOneNanoMole;

%%%%%%%%%%%%%
llikp_tmp = 0;
for tt = 1:n_T
    
  llikp_tmp = llikp_tmp + log(mvnpdf(mYobs(tt,:), mYode_tmp(tt,:), diag(inputp_tmp((end-n_Y+1):end))));
  %put the var(y) in the last
