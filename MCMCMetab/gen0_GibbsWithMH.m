...
%
% only have 24h observations, but with multiple independent replicates for this time
% point
%
% mmYobs: a matrix, one row for one observation of 24h
%
%



function [ ps, ps_trial, chi2s, chi2s_trial, acceptance ] = Metab_GibbsWithMH(mmYobs, Yode, startp, muprior,sigmaprior, indexFit, nruns, nburnin, nthinning,Xd,observed,MoleculeNumberInOneNanoMole,ode_fun)
...
%n_ty = size(mmYobs,1);  % number of observations for this 24h
N_tmp = length(startp);
n_Y = length(Yode);
...
llikp_tmp = sum(log(mvnpdf(mmYobs, Yode_tmp, diag(inputp_tmp((end-n_Y+1):end))))); %put the var(y) in the last
