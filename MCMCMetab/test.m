...
[ps, ps_trial, chi2s, chi2s_trial, acceptance ] = Metab_GibbsWithMH(Yobs, Yode, startp, muprior,sigmaprior, indexFit,2000, 500, 5,Xd,observed,MoleculeNumberInOneNanoMole,ode_fun);

sample_mm(seed,:) = mean(ps(:,1:8));
variation_m(seed,:) = (sample_mm(seed,1:6)-OptimizeParameters(1:6))./sqrt(sigmaprior(1:6));
end

fileID = fopen('gibbs01to30Par3to4.txt','w');
fprintf(fileID,'%12s\n','samplemean');
